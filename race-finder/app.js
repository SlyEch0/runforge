...
// CF Pages Functions proxy preferred; jina.ai CORS fallback for RunSignUp
...
function parseJinaJson(text) {
  ...
  const body = idx >= 0 ? text.slice(idx + marker.length).trim() : text.trim();
  const start = body.indexOf('{');
  const end = body.lastIndexOf('}');
  if (start < 0 || end <= start) throw new Error('No JSON in proxy response');
  return JSON.parse(body.slice(start, end + 1));
}

async function fetchViaJina(absoluteUrl) {
  const httpUrl = absoluteUrl.replace('https://', 'http://');
  const res = await fetch('https://r.jina.ai/' + httpUrl);
  if (!res.ok) throw new Error('jina HTTP ' + res.status);
  return parseJinaJson(await res.text());
}

async function fetchRsuJson(pathAndQuery) {
  const directUrl = RSU_DIRECT + '/' + pathAndQuery;
  try {
    if (pathAndQuery.startsWith('race/')) {
      return await fetchJson(RSU_RACE + pathAndQuery.slice('race/'.length));
    }
  ...
  } catch (e) { console.warn('CF proxy failed', e); }
  try {
    return await fetchViaJina(directUrl);
  } catch (e) { console.warn('jina failed', e); }
...
async function enrichRsu(race) {
  ...
  try {
    ...
    return normalizeFromRsu(data.race || data);
  } catch {
    return race;
  }
}
...
function mergePreferRsu(rsuList, rfList) {
  ...
  for (const r of rsuList) byKey.set(r.key, r);
  const byId = new Map();
...
async function geocodePlace(city, state, zip) {
  ...
  try {
    ...
    if (state) {
      ...
      const full = adminMap[state.toUpperCase()] || state;
...
async function searchRaces() {
  ...
  try {
    ...
    for (const r of merged) r._coords = await coordsFor(r);
    const uniqueCoords = [];
    const seenC = new Set();
